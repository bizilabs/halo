...
internal fun Modifier.drawAxesAndGridLines(
    ...
    // gets only one set of the x-axis
    val xAxisLabels = data
        .map { it.xValue }
        .distinct()
    ...
    if (labelConfig.showXLabel) {
        ...
        xAxisLabels.forEachIndexed { index, label ->
            val textCharCount = labelConfig.getXLabelTextCharCount(
                xValue = label,
                displayDataCount = xAxisLabels.count()
            )
            val textSizeFactor = if (xAxisLabels.count() <= 13) 70 else 90

            val textLayoutResult = textMeasurer.measure(
                text = label.toString().take(textCharCount),
